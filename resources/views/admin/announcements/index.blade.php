...
{{-- Displaying all threads created by user --}}
    ...
    <a href="{{route('admin.announcements.create')}}" class="btn btn-primary px-2">Create Announcement</a>
      ...
      <table class="table table-bordered mt-3" id="normal-table" style="table-layout:auto">
        ...
        @forelse($announces as $announce)
          ...
          <tbody>
            <tr>
              {{-- Link to show page --}}
              <td class="p-3"><a href="{{route('admin.announcements.show',$announce->id)}}" id="counter" class="link-underline link-underline-opacity-0 link-underline-opacity-100-hover text-black">{{$announce->title}}</a></td>
              {{-- User name --}}
